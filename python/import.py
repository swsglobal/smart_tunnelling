...
if not os.path.isfile(sDBPath):
    ...
    exit(1)
sGeoReliability_XLS = os.path.join(os.path.abspath('..'), bbtConfig.get('Import', 'folder'),
                                   bbtConfig.get('Import', 'valutazione'))
if not os.path.isfile(sGeoReliability_XLS):
    print "Errore! File %s inesistente!" % sGeoReliability_XLS
    exit(2)
sCE_XLS = os.path.join(os.path.abspath('..'), bbtConfig.get('Import', 'folder'),
                       bbtConfig.get('Import', 'geo'))
if not os.path.isfile(sCE_XLS):
    print "Errore! File %s inesistente!" % sCE_XLS
    exit(3)
sProfilo_XLS = os.path.join(os.path.abspath('..'), bbtConfig.get('Import', 'folder'),
                            bbtConfig.get('Import', 'profilo'))
...
for row_idx in range(3, xl_sheet.nrows):
    ...
    for reli in reliab_match:
        print "reliab %d-%s finisce in %f vicino a Geoseg %f" % (reli.id, reli.gmr_class,
                                                                 reli.fine, geoseg.fine)
...
# aghensi@20160531 - utilizzo xlsx creato da plugin civil
...
######## BbtParameter rappresenta il segmento unitario con i parametri geologici base
######## valorizzati secondo i dati provenienti dalle tavole geologiche
...
for k in geoitems:
    ...
    for i, pxi in enumerate(px):
        geosec = gx[i]
        for j in range(prev, prev+pxi):
            pk = geoitems[k][j]
            tmparr = [pk.inizio, pk.fine, pk.est, pk.nord, pk.he, pk.hp, pk.co, pk.hw, pk.wdepth,
                      geosec.g_med, geosec.g_stddev, geosec.phimin, geosec.phimax, geosec.ei_med,
